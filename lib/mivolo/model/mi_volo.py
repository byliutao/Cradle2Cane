...
from lib.mivolo.data.misc import prepare_classification_images
from lib.mivolo.model.create_timm_model import create_model
from lib.mivolo.structures import PersonAndFaceCrops, PersonAndFaceResult
...
class MiVOLO:
    ...
    def inference(self, model_input: torch.tensor, return_feature=False) -> torch.tensor:

        # with torch.no_grad():
        if self.half:
            model_input = model_input.half()
        if return_feature:
            output = self.model.forward_features(model_input)
        else:
