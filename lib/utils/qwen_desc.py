...
# ==== 主函数：批量描述人脸特征 ====
def predict_face_description_for_folder(
    image_folder,
    output_csv,
    qwen_model,
    qwen_processor,
    ...
    for fname in tqdm(all_filenames, desc="Processing images"):
        ...
        query = (
            "Describe the facial features of this person, especially focusing on skin color, hair color, eye shape, and any other distinctive traits."
        )
        try:
            output = batch_infer([query], [image], qwen_model, qwen_processor, device)
            result_rows.append([fname, output[0]])
        except Exception as e:
            print(f"Error processing {fname}: {e}")
            result_rows.append([fname, f"error: {str(e)}"])
...
if __name__ == "__main__":
    image_folder = "others/cele_test_wo_bg"  # 替换为你的图片目录
    output_csv = "others/cele_test_wo_bg/face_descriptions.csv"
    ...
    predict_face_description_for_folder(image_folder, output_csv, model, processor, device)
